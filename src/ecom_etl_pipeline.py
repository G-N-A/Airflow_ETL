...
class EcommerceETL:
    ...
    def clean_orders(self, df: DataFrame) -> DataFrame:
        ...
        df_cleaned = df_cleaned.withColumn(
            ...
            when((col('total_amount') - col('calculated_total')).cast("double") > 0.01, True).otherwise(False)
        )
    ...
    def clean_order_items(self, df: DataFrame) -> DataFrame:
        ...
        df_cleaned = df_cleaned.withColumn(
            ...
            'line_total_mismatch',
            when((col('line_total') - col('calculated_line_total')).cast("double") > 0.01, True).otherwise(False)
        )
    ...
    def create_customer_360(self, customers_df: DataFrame, orders_df: DataFrame, 
        ...
        order_metrics = orders_df.groupBy('customer_id').agg(
            ...
            concat_ws(',', collect_set('payment_method')).alias('payment_methods_used'),
            concat_ws(',', collect_set('placed_via')).alias('channels_used')
        )
        ...
        click_metrics = clickstream_df.filter(col('customer_id').isNotNull()).groupBy('customer_id').agg(
            ...
            concat_ws(',', collect_set('device')).alias('devices_used'),
    ...
    def load_to_target(self, df: DataFrame, table_name: str, mode: str = 'overwrite'):
        ...
        try:
            ...
            # Convert any array columns to strings to avoid JDBC type issues
            df_to_save = df
            for field in df.schema.fields:
                if str(field.dataType).startswith('ArrayType'):
                    logger.info(f"Converting array column {field.name} to string for SQL Server compatibility")
                    df_to_save = df_to_save.withColumn(field.name, col(field.name).cast("string"))
